...
@Form.create()
export default class CountTasks extends PureComponent {
  constructor(props){
    super(props);
    // 出事状态
    this.state = {
  ...
  render() {
    ...
    const columns = [{
      ...
      title: '正常数',
      key: 'p_constant',
      dataIndex: 'p_constant'
    },{
      title: '异常数',
      key: 'e_constant',
      dataIndex: 'e_constant'
    },,{
      title: '操作',
      key:'operating',
      dataIndex:'operating',
      render: (key,record) =>(<div><Button type="primary" size='small' onClick={() => this.handleTableEdit(record)} style={{marginRight:'.2rem'}}>修改</Button><Button type="primary" size='small' onClick={() => this.handleTableEdit(record)} style={{marginRight:'.2rem'}}>明细</Button><Button type="danger" size='small' style={{marginRight:'.2rem'}}>删除</Button></div>)
    }];
    // 模拟数据
    const data = [{
      key: '1',
        ...
        <Card bordered={false}>
          <div className={styles.tableList}>
            ...
            <div className={styles.tableListOperator}>
              <Button icon="plus" type="primary" onClick={this.handleAddAssets.bind(this)}>添加盘点</Button>
            </div>
          {/* 表格信息 */}
            <TableList
              loading={false}
