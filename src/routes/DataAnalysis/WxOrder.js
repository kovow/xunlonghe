...
@Form.create()
class WxOrder extends PureComponent {
  constructor(props){
    super(props);
    // 初始状态
    this.state = {
  ...
  handleDownExcel = () => {
    ...
    if(data){
      ...
      // .then(excel=>{
        ...
        export_json_to_excel(tHeader, list, '微商城交易数据'+date);
      // }).then(()=>{
  ...
  handleDownDetailExcel = ()=>{
    ...
    if(wy){
      ...
      // .then(excel=>{
        ...
        export_json_to_excel(tHeader, list, '微商城交易明细'+date);
      // }).then(()=>{
        message.success('下载成功');
  ...
  componentWillMount() {
    ...
    // // 获取商家名称
    dispatch({
      type: 'analysis/fetchShopNameOne'
    });
  }
  render() {
    // 表格数据
    ...
    return (
      <PageHeaderLayout title="微商城交易数据">
        <Card bordered={false}>
